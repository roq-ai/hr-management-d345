...
import { getVacancies } from 'apiSdk/vacancies';
...
function DashboardPage() {
  ...
  const vacancyStat = useSWR<PaginatedInterface<VacancyInterface>>(
    () => `/vacancies`,
    () => getVacancies({ limit: 1, offset: 0 }),
  );

  const profileSearchStat = useSWR<PaginatedInterface<ProfileSearchInterface>>(
    () => `/profile-searches`,
    () => getProfileSearches({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Vacancies',
      result: vacancyStat?.data?.totalCount,
      isLoading: vacancyStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Profile Searches',
      result: profileSearchStat?.data?.totalCount,
      isLoading: profileSearchStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
